...
| Scroll animation | Apple Web uses `350ms easeInOutQuad`; current-index changes call `updateScroll()` once, while the native renderer preserves velocity across fast retargets | `performAppleScrollTop()` owns one fast near-critical rAF spring writer: stiffness `320`, damping `34`, position threshold `.5px`, velocity threshold `5px/s`, with a `700ms` safety cutoff. Normal cuts visually settle around Apple's compact `350ms` window. The full list does not visibly overshoot its anchor; the next four rows below the target reuse the same spring progress with `4.5%` per-row delay capped at `18%`, producing the sentence handoff without starting independent row tweens. Only source/target rows hand off alpha and scale, so blurred inactive rows never flash brighter while passing the focus anchor |
...
- The desktop backdrop combines four sampled cover-edge colors into a large radial/linear field and overlays a heavily blurred same-cover artwork layer (`opacity=.82`, `blur(128px)`, `brightness(.72)`). This follows Apple's full-screen Mac composition: recognisable artwork colours without readable image detail behind lyrics.
- The desktop veil is a restrained dark vertical gradient and the masked cover halo keeps Apple radiosity opacity (`APPLE_DESKTOP_COVER_HALO_OPACITY = 0.4`), preserving white-text contrast without flattening the cover palette.
...
- Native-style desktop line cuts use one fast near-critical retained-velocity spring (`stiffness=320`, `damping=34`) for scroll plus the outgoing/incoming scale and main lyric alpha handoff. Inactive blurred rows keep fixed alpha; the recognisable Apple spring comes from the shared row cascade (`4.5%` per lower row, capped at `18%`).
...
- Poster dissolve mask is retained for the user-required "poster dissolve" behavior. The clear artwork shadow and `0.4` radiosity halo opacity are Apple CSS evidence; the extra cover mask itself is not claimed as a fixed Apple fullscreen CSS constant. The full-screen artwork layer must stay heavily blurred so it contributes only colour masses, never crisp poster detail behind lyrics.
...
- Asserts cover-derived backdrop gradients, blurred artwork/veil balance, capped artwork/halo opacity, poster dissolve
...
- Previous browser baseline artifact (its flat-backdrop assertions are superseded):
  `/var/folders/gv/y_6ml9zd1fgdpt1yggmb04140000gn/T/claudio-apple-lyrics-verify-1782703936146`
- Current backdrop contract: the root remains opaque, while the backdrop uses
  multiple cover-edge gradients plus a heavily blurred same-cover layer. The
  lyric column itself remains transparent and `plus-lighter`; readability
  comes from blur and the restrained full-screen veil rather than a separate
  black rectangle behind the lyrics.
- Current line-transition implementation contract:
  - `.line` keeps `padding 0.1s ease-in-out, height 0.4s linear, margin-top 0.4s linear`; outgoing/incoming scale and main vocal alpha are written from the shared spring and therefore have no independent `transform` or `color` tween. Other inactive rows stay dim and blurred throughout the scroll.
