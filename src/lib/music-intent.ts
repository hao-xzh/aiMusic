...
const QueueIntentSchema = z.object({
  action: z.enum(["insert", "replace"]).default("replace"),
...
- "insert"：用户只指名一首歌想听（"放浮夸"、"来一首七百年后"、"插一首 XX"）。
  当前队列保留，新歌插到下一首并立刻播；新歌结束后回到原本的下一首。
- "replace"（默认）：用户指定一个主题/场景/艺人探索（"听陈奕迅"、"放点蓝调"、
  "想听陈奕迅从七百年后开始"、"排几首慢的"、"陪我熬夜"）。把整个队列换成新的。
- 判定要点：单歌名 → insert；艺人 / 艺人+起始歌 / 情绪场景 → replace。不确定就 replace。
...
function normalizeIntent(intent: MusicIntent, queryText: string): MusicIntent {
  ...
  const queueIntent = { ...intent.queueIntent };
  const refs = {
  ...
  if (/越来越带感|递进|越放越/.test(q)) queueIntent.orderStyle = "energy_up";
  if (/睡觉|睡眠|入睡/.test(q)) {
    queueIntent.orderStyle = "sleep";
    soft.energy = "low";
    soft.scenes = add(soft.scenes, "sleep");
  }
  queueIntent.action = inferQueueAction(queryText, intent, hard);
...
function inferQueueAction(
  queryText: string,
  intent: MusicIntent,
  hard: HardConstraints,
): MusicIntent["queueIntent"]["action"] {
  if (intent.queueIntent.action === "insert") return "insert";
  const q = queryText.toLowerCase().trim();
  const trackHints = unique([...hard.tracks, ...intent.textHints.tracks]);
  const artistHints = unique([...hard.artists, ...intent.textHints.artists]);
