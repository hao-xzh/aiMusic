...
class QueueValidator(
    ...
    fun validateStructured(actions: List<PlannedAction>, userText: String = ""): QueueValidation {
        ...
        if (tracks.any { !app.pipo.nativeapp.data.FunctionalMusicFilter.acceptsCategory(it, userText) }) {
            messages.add("religious_category_not_requested")
        }
    ...
    fun validate(userText: String, actions: List<PlannedAction>): QueueValidation {
        ...
        val messages = mutableListOf<String>()
        if (tracks.any { !app.pipo.nativeapp.data.FunctionalMusicFilter.acceptsCategory(it, userText) }) messages.add("religious_category_not_requested")
        var primarySatisfied = true
        var mustIncludeSatisfied = true
        var closerSatisfied = true
        val catalogSatisfied = validateCatalogScope(play, messages)
