...
import app.pipo.nativeapp.data.agent.normalize.FavoriteRequestPolicy
...
class AgentToolLoop(
    ...
    suspend fun run(
        ...
        if (isRetryOnlyRequest(input.userText)) {
            val pending = ledger.pendingFavorite()
            state.retryFavorite = pending?.takeIf {
                it.accountUserId == repository.account.first()?.userId &&
                    state.startedAtMs - it.createdAtMs in 0L..3_600_000L
            }
            if (pending != null && state.retryFavorite == null) ledger.clearFavoriteRetry()
        } else {
            // A new request supersedes the previous retry context; chat/tool text cannot revive it.
            ledger.clearFavoriteRetry()
        }
        if (state.retryFavorite != null) {
            // Continue the already authorized action with its recorded target and direction.
            // Selecting a fresh tool/target through the model would make this retry ambiguous.
            state.trace("retry_recorded_favorite")
            retryFavorite(input, executor, state)
            return buildOutcome(input, state, "上次收藏记录已变化，请明确要操作的歌曲。")
        }
...
""".trimIndent()
        ...
        val observation = runCatching {
            when (call.name) {
                ...
                "like_current" -> commitFavorite(
                    ...
                    like = call.arguments.optBoolean("like", true),
                    execute = { executor.likeCurrent("like", call.arguments.optBoolean("like", true)) },
                )
    ...
    private suspend fun likeTrack(
        ...
        return commitFavorite(
            state = state,
            plan = plan,
            like = like,
            execute = { executor.likeTrack("like_track", like, requirement) },
        )
    }

    private suspend fun commitFavorite(
        state: LoopState, plan: MusicTurnPlan, like: Boolean,
        execute: suspend () -> ActionExecutionResult,
    ): JSONObject {
        val accountId = repository.account.first()?.userId
        val result = execute()
        if (result.success) {
            ledger.clearFavoriteRetry()
            state.retryFavorite = null
        } else {
            val target = result.tracks.singleOrNull()
            if (accountId != null && repository.account.first()?.userId == accountId && target?.neteaseId != null) {
                ledger.saveFailedFavorite(state.turnId, accountId, target, like)
            }
            if (!hasLikelyMultipleActions(plan.userText)) state.done = true
        }
        return commitSimple(state, plan, QueueValidation(true), result)
    }

    private suspend fun retryFavorite(
        input: AgentTurnInput, executor: AgentActionExecutor, state: LoopState,
    ): JSONObject {
        val pending = state.retryFavorite ?: return JSONObject().put("ok", false).put("error", "no_pending_favorite")
        if (ledger.pendingFavorite()?.originTurnId != pending.originTurnId) {
            return JSONObject().put("ok", false).put("error", "favorite_retry_superseded")
        }
        val target = TrackRequirement(pending.track.title, pending.track.artist)
        val plan = MusicTurnPlan(state.turnId, input.userText,
            listOf(PlannedAction.LikeTrack("retry_favorite", pending.like, target)),
            isRepair = true, repairTargetTurnId = pending.originTurnId, plannerRaw = "tool_loop")
        return commitFavorite(state, plan, pending.like) {
            executor.retryFavorite("retry_favorite", pending.track, pending.like, pending.accountUserId)
        }
    }
    ...
    private suspend fun buildUserPrompt(input: AgentTurnInput, state: LoopState): String {
        val context = AgentContextBuilder(ledger).build(
            input,
            ...
            userTaste = app.pipo.nativeapp.data.PipoGraph.userTaste.context(),
        )
        if (!isRetryOnlyRequest(input.userText)) return context
        val retry = JSONObject().put("available", false)
        return context + "\n<favorite_retry_data>\n$retry\n</favorite_retry_data>"
    }
...
- 普通“安静、舒缓、放松”描述音量感、能量和编曲密度，不等于宗教、佛教、大悲咒、助眠流水线、白噪音或纯音乐。保留既有语言/人声要求，选真实适合的歌曲。只有用户明确请求相应类别才可选择；宗教题材不能从口味库或模型联想擅自引入。
- 不把本次临时情绪写成稳定偏好；用户当前明确要尝试平时不喜欢的风格时照本次要求执行。
